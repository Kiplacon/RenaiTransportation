...
local function click(event)
	...
	if (clicked) then
		...
		if (settings.startup["RTItemCannonSetting"].value == true and clicked.name == "RTItemCannon") then
			player.opened = storage.ItemCannons[script.register_on_object_destroyed(clicked)].chest

		elseif (settings.startup["RTTrainRampSetting"].value == true) then
			if ((clicked.name == "RTMagnetTrainRamp" or clicked.name == "RTMagnetTrainRampNoSkip" or clicked.name == "RTMagnetSwitchTrainRamp" or clicked.name == "RTMagnetSwitchTrainRampNoSkip") 
			and PlayerProperties.SettingRampRange.SettingRange == false) then
				local ramp = clicked
